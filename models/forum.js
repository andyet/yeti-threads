...
module.exports = function (gatepost) {
  ...
  const Forum = new gatepost.Model({
    id: {
  ...
  const ForumTree = new gatepost.Model({
    results: {type: 'array'}
  });
  ...
  const IdArray = new gatepost.Model({
    ids: {
      validate: joi.array(),
  ...
  IdArray.fromSQL({
    name: "getIds",
    sql: (args) => SQL`SELECT json_agg(forums.id) AS ids FROM forums JOIN forums_access ON forums.id=forums_access.forum_id WHERE (forums_access.user_id=${args.arg} AND forums_access.read=True) OR forums.owner=${args.user}`,
    oneResult: true
  });
  ...
  Forum.fromSQL({
    name: 'delete',
    sql: (args, model) => SQL`DELETE FROM forums WHERE id=${args.forum_id} AND owner=${args.user} returning id`,
    oneResult: true,
  });
  ...
  count(forum_rows.*)::INTEGER as count
